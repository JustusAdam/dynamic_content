...
class RegionOperations(Operations):
    ...
    def add_item_conf(self, element_name, element_type, handler_module, show_title=True):
        self.execute('add_item_config', element_name=escape(element_name), element_type=escape(element_type), handler_module=escape(handler_module), show_title=escape(show_title))
