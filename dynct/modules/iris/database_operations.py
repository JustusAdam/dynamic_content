from dynct.core.database_operations import Operations, ContentTypes
from dynct.backend.database import escape
...
class Pages(Operations):
    ...
    def add_page(self, page_type, content_type, page_title, creator, published):
        values = dict(page_type=page_type, content_type=escape(content_type), page_title=escape(page_title),
                      creator=escape(creator), published=escape(published))
...
class Fields(Operations):
    ...
    def add_field_type(self, machine_name, field_name, content_type, handler_module, description=''):
        self.execute('add_field_type', machine_name=escape(machine_name), field_name=escape(field_name),
                     content_type=escape(content_type), handler_module=escape(handler_module),
