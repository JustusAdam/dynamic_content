from dynct.core.database_operations import Operations, escape
...
class MenuOperations(Operations):
    ...
    def add_menu_item(self, item_name, item_path, menu, enabled, parent_item, weight, tooltip=''):
        if not tooltip:
            tooltip = item_path
        self.execute('add_menu_item', item_name=escape(item_name), item_path=escape(item_path), menu=escape(menu),
                     enabled=escape(enabled), tooltip=escape(tooltip), parent_item=escape(parent_item),
